...
def run(gpu, ngpus_per_node, args, log_queue):
    ...
    fg_adj = data.get_struct(args.dataset,filename='adj.npz')
    fg = DGLGraph(fg_adj, readonly=True)
    del fg_adj
    # fg_adj = data.get_struct(args.dataset,filename='adj_bak.npz')
    # fg_labels = data.get_labels(args.dataset)
    fg_train_mask, _, _ = data.get_masks(args.dataset)
    ...
    with torch.autograd.profiler.profile(enabled=(args.gpu == 0), use_cuda=True) as prof:
        with torch.autograd.profiler.record_function('total epochs time'):
            for epoch in range(args.epoch):
                ...
                sampler = dgl.contrib.sampling.NeighborSampler(fg, args.batch_size, expand_factor=int(sampling[0]), num_hops=len(sampling)+1, neighbor_type='out', shuffle=False, num_workers=args.num_worker, seed_nodes=train_lnid, prefetch=True, add_self_loop=True)
                ...
                for nf in sampler:
                    # subgraph_fname = os.path.join(f'{args.dataset}/nfs/', f'{epoch}_{iter}.pkl')
                    wait_sampler.append(time.time()-st)
                    offsets = nf._layer_offsets
                    nf_nids = nf._node_mapping.tousertensor()
                    tnid = nf_nids[offsets[0]:offsets[1]]
                    logging.info(f'{train_lnid}')
                    logging.info(f'{len(nf._layer_offsets) - 1}')
                    logging.info(f'nf{epoch}_{iter}: {tnid.shape[0]},{tnid}')
                    # get_memory_usage(f'{epoch}_{iter}')
...
if __name__ == '__main__':
    ...
    if len(args.sampling.split('-')) > 10:
        ...
        log_filename = os.path.join(log_dir, f'get_nfs_{model_name}_{datasetname}_trainer{args.world_size}_bs{args.batch_size}_sl{fanout}x{sampling_len}_ep{args.epoch}_hd{args.hidden_size}.log')
    else:
        log_filename = os.path.join(log_dir, f'get_nfs_{model_name}_{datasetname}_trainer{args.world_size}_bs{args.batch_size}_sl{args.sampling}_ep{args.epoch}_hd{args.hidden_size}.log')
