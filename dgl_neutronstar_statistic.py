...
import storage
from model import graphsage, deep
# from model import gcn_nstar
...
def run(gpu, ngpus_per_node, args, log_queue):
    ...
    if args.model_name == 'gcn':
        # model = gcn_nstar.GCNSampling(featdim, args.hidden_size, args.n_classes, len(
        #     sampling), F.relu, args.dropout, cache_client.Nid2pid, args.rank)
        ...
        with torch.autograd.profiler.record_function('total epochs time'):
            for epoch in range(args.epoch):
                ...
                for nf in sampler:
                    ...
                    total_delete_edge = 0
                    total_edges =  nf._block_offsets[-1]
                    total_nodes = total_edges + args.batch_size - nf._block_offsets[1]
                    ...
                    nf_layers = nf.num_layers
                    for i in range(nf_layers-1):
                        print(f'i = {i}')
                        edge_ids = nf.block_edges(i)[1]
                        dst_nids, counts = torch.unique_consecutive(ori_nid[edge_ids], return_counts=True)
                        dst_bool = torch.from_numpy(cache_client.Nid2pid[dst_nids] == args.rank)
                        # 要从远程通信获得的元素的下标
                        false_indices = torch.nonzero(~dst_bool, as_tuple=True)[0]
                        total_delete_edge += sum(counts[false_indices])
                        # print(f'false_indices = {false_indices.shape} {false_indices[0]}')
                        delete_dst_nid.append(false_indices)
                    
                    total_delete_node = sum([len(item) for item in delete_dst_nid])
                    logging.info(f'ori #nodes = {total_nodes}, #edges = {total_edges}')
                    logging.info(f'nstart #nodes = {total_nodes - total_delete_node}, #edges = {total_edges - total_delete_edge}')
                    
                    ed = time.time()
                    logging.info(f'using time: {ed-st}')
                    sys.exit()
...
if __name__ == '__main__':
    ...
    if len(args.sampling.split('-')) > 10:
        ...
        log_filename = os.path.join(log_dir, f'neutronstar_{model_name}_{datasetname}_trainer{args.world_size}_bs{args.batch_size}_sl{fanout}x{sampling_len}_ep{args.epoch}_hd{args.hidden_size}_localFalse.log')
    else:
        log_filename = os.path.join(log_dir, f'neutronstar_{model_name}_{datasetname}_trainer{args.world_size}_bs{args.batch_size}_sl{args.sampling}_ep{args.epoch}_hd{args.hidden_size}_localFalse.log')
